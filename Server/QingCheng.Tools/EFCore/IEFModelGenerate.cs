...
namespace QingCheng.Tools.EFCore
{
    ...
    public class TableMeta
    {
        ...
        public List<TableColMeta> Column { get; set; } = new List<TableColMeta>();
    ...
    public class MySqlEFModelGenerate : IEFModelGenerate
    {
        ...
        public string GenerateModel(string tableName, List<TableColMeta> meta)
        {
            ...
            foreach (var col in list)
            {
                ...
                if (string.IsNullOrWhiteSpace(col.COLUMN_KEY) == false)
                {
                    sb.AppendLine("[Key]");
                }
        ...
        public async Task<List<TableColMeta>> GetAllTable(string dbName, string connString)
        {
            ...
            conn.Open();
            var res = await conn.QueryAsync<TableColMeta>($"select * from information_schema.COLUMNS where TABLE_SCHEMA = '{dbName}'");
            return res.ToList();
        }
    }
}
