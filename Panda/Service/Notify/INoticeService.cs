...
public class NoticeService : INoticeService
{
    ...
    public async Task<IEnumerable<NoticeResponse>> GetNoticeByCache(int top)
    {
        ...
        {
            var list = await _noticeRepository.Queryable().OrderByDescending(a => a.IsTop)
                .ThenByDescending(a => a.AddTime).Take(top).ProjectToType<NoticeResponse>().ToListAsync();
            return list;
    ...
    {
        var list = await _noticeRepository.Queryable().Page(request).OrderByDescending(a => a.IsTop)
            .ThenByDescending(a => a.AddTime).ProjectToType<NoticeResponse>().ToListAsync();
        return list;
    }
