...
import { defineComponent, render, PropType, ref, onMounted, reactive } from "vue";
...
const Test2 = (props: { name: string }) => {
    ...
    return <>
        <h2>Test2:{props.name} - {userName.value} - 状态:{status.value}</h2>
        <ElInput v-model={userName.value}></ElInput>
        <ElSwitch v-model={status.value}></ElSwitch>
        <h1 style={{ background: '#ccc', display: 'inline-block',userSelect:'none' }} onMousedown={
            e => {
                console.log(e.clientX, e.clientY, e.screenX, e.screenY)
                let el = e.currentTarget as HTMLElement;
                console.log(el.style.marginLeft)
                el.style.marginLeft = `${(el.style.marginLeft ? parseInt(el.style.marginLeft) : 0) + 10}px`;
            }
        }>点击我</h1>
        <br />
    </>
                    ...
                    if (item.control == 'select') {
                        ...
                        return <div>{
                            control.items?.map(config => {
                                return <p>配置:{config.label} - {config.value}</p>
                            })
                        }</div>
                    }
                    return null
