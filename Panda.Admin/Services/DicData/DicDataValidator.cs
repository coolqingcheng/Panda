...
public class DicDataValidator
{
    ...
    private void Init()
    {
        _dicDataRequests.Add(new DicDataRequest(
            new DicDataGroupInfo("tencent_cos", "腾讯云COS"),
            new List<DicDataChildInfo>
            {
                new("cos_region", "COS 地域的简称"),
                new("secret_id", "云 API 密钥 SecretId"),
                new("secret_key", "云 API 密钥 SecretKey", true),
                new("bucket", "存储桶名称，此处填入格式必须为 bucketname-APPID"),
                new("host", "访问默认域名")
            }));
    ...
    public DicDataRequest Validate(DicUpdateRequest request)
    {
        ...
        if (dicItem == null) throw new UserException("groupKey不能为空");
